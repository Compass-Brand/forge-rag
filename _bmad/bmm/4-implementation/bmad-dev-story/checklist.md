...
- [ ] **Action Evidence Matches Story Type:** For create/deploy/reset/init/recreate stories, completion evidence shows the required action actually happened in the required starting state during this run, not just that an already-populated repo currently looks correct
...
- [ ] **Validation-Only Evidence:** For docs-only or validation-only stories, explicit verification commands/results are captured in place of code tests where appropriate
