...
If the document exists and has frontmatter with `stepsCompleted`, follow the Continuation Protocol. Step 01b is responsible for deciding whether the workflow should resume or report that it is already complete:
...
"I've set up the PRD workspace for this repo.
...
📋 **Note:** This is a **brownfield project**. Existing substantive project documentation has been loaded, so the next step should focus on changes to an existing system.
{else}
📋 **Note:** This is a **greenfield project**. Scaffold-only docs do not change that classification. The next step should build from the product brief and the repo's stated purpose.
{/if}
