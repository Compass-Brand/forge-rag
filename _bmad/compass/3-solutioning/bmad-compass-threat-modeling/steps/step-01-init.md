...
- Look for file at `{current_threat_modeling_dir}/threat-model-{project_name}.md`
...
**Check `stepsCompleted` only after evaluating `workflowComplete`:**

- If `workflowComplete` is false and `stepsCompleted` exists and is not empty → **STOP here and load `{continueFile}` immediately**
