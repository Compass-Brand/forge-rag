...
outputFile: '{current_security_review_dir}/security-review-{project_name}.md'
...
- Look for file at `{current_security_review_dir}/security-review-{project_name}.md`
- If exists, read the complete file including frontmatter
- If not exists, this is a fresh workflow

### 2. Handle Completed Workflow
...
- Check the completed-workflow branch first
- If the workflow is not complete and `stepsCompleted` exists and is not empty, **STOP here** and load `{continueFile}` immediately
...
- Look for: `{current_architecture_dir}/*.png`
- Look for: `{current_architecture_dir}/*.pdf`
- Look for: `{current_architecture_dir}/*.md` (excluding security-review files)
...
6. **Architecture Diagrams**: [If found during discovery: "I found [X] architecture diagrams in the current architecture lane. Would you like me to reference them?" | If not found: "Do you have architecture diagrams (C4, UML, informal) you'd like to share?"]
