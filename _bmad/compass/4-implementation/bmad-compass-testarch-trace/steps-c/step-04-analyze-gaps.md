...
if (criticalGaps.length > 0) {
  recommendations.push({
    priority: 'URGENT',
    action: `Run /bmad-tea-testarch-atdd for ${criticalGaps.length} P0 requirements`,
...
if (highGaps.length > 0) {
  recommendations.push({
    priority: 'HIGH',
    action: `Run /bmad-tea-testarch-automate to expand coverage for ${highGaps.length} P1 requirements`,
...
recommendations.push({
  priority: 'LOW',
  action: 'Run /bmad-tea-testarch-test-review to assess test quality',
  requirements: [],
});
```
