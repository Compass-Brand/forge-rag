...
dimensions.forEach((dim) => {
  const outputPath = `{test_artifacts}/tmp/tea-test-review-${dim}-${timestamp}.json`;
...
fs.writeFileSync(`{test_artifacts}/tmp/tea-test-review-summary-${timestamp}.json`, JSON.stringify(reviewSummary, null, 2), 'utf8');
```
