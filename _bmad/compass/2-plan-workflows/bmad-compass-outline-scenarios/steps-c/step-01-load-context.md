...
- `current_outline_scenarios_dir`
- `current_ux_agent_dialog_dir`
...
Read `{current_trigger_mapping_dir}/trigger-map.md` (the hub document)
...
- `{current_trigger_mapping_dir}/02-*.md` (Primary persona)
- `{current_trigger_mapping_dir}/03-*.md` (Secondary persona)
- `{current_trigger_mapping_dir}/04-*.md` (Tertiary persona, if exists)

### 4. Check for Existing Work

**Check for resume situation:**
- Does `{current_outline_scenarios_dir}/` exist?
- Are there any scenario files already?
- Are there unfinished agent dialogs in `{current_ux_agent_dialog_dir}/`?
