...
- 💾 Save delivery file to `{current_design_delivery_dir}/DD-XXX-name.yaml`
...
- Create file at `{current_design_delivery_dir}/DD-XXX-name.yaml`
- Fill out basic metadata:
  - `id`: DD-XXX
  - `name`: Descriptive flow name
  - `type`: user_flow / feature / component
  - `status`: ready
  - `priority`: high / medium / low
  - `created_by`: User name or role from config
...
- List all scenarios included (reference `{current_outline_scenarios_dir}/` files)
- List user flows covered
- List conceptual specification files used (reference `{current_conceptual_specifications_dir}/`)
...
- Estimate size and effort using the template fields (`size` and `effort`)
...
<output>Design Delivery file created: `{current_design_delivery_dir}/DD-XXX-name.yaml`</output>
