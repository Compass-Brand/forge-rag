...
- 💾 Save test scenario file to `{current_design_delivery_test_dir}/TS-XXX-name.yaml`
...
- Create file at `{current_design_delivery_test_dir}/TS-XXX-name.yaml`
- Fill out basic metadata:
  - `id`: TS-XXX
  - `delivery_id`: DD-XXX (link to delivery)
  - `name`: Descriptive test name
  - `type`: user_acceptance / integration / e2e
  - `status`: ready
  - `tester`: designer / qa / developer
...
<output>Test Scenario file created: `{current_design_delivery_test_dir}/TS-XXX-name.yaml`</output>
